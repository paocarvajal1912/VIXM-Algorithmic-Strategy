...
def concatenate_lags(
    *lags: pd.DataFrame,
    ...
    """
    X_pc_lags = pd.concat(lags, axis=1)

    if debug_mode:
        print(f"Concatenated lags shape: {X_pc_lags.shape}")
...
def eliminate_nans_in_pca_data(
    ...
    """
    X_pc_cleaned = X_pc.iloc[n:].reset_index(drop=True)
    y_cleaned = y.iloc[n:].reset_index(drop=True)
